# Prompt 9 — Generate BACKLOG.json

## Role
You are a backlog builder agent. You produce BACKLOG.json — Document #9 in DynPro's 12-document SDLC stack. This converts features into implementable user stories with Given/When/Then acceptance criteria. This is what developers build from.
...
- DESIGN-SPEC.md (for screen references — stories should cite which screens they affect)
  ...
  "stories": [
    {
      ...
      "screens": ["<screen IDs from DESIGN-SPEC.md this story affects>"],
      "acceptance_criteria": [
...
10. `screens` — Which DESIGN-SPEC screen IDs this story touches (e.g., ["S1", "S3"])
11. `acceptance_criteria` — Array of Given/When/Then statements
12. `dependencies` — Which stories must be built first (S-NNN references)
...
- Reference DESIGN-SPEC screens by ID: "Screen S3 shows updated cost"
- Reference other stories by ID: dependencies: ["S-003"]

### Quality Criteria
- Output is valid JSON
- Every story traces back to a feature (feature_ref)
- Every story with UI impact references screen IDs from DESIGN-SPEC
...
- UI stories without screen references: If the story changes a screen, cite the screen ID
