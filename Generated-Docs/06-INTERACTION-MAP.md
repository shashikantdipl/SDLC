...
**Document:** 6 of 14 | Status: Draft
**Role:** This is the CONTRACT between MCP-TOOL-SPEC (Doc 7) and DESIGN-SPEC (Doc 8). Both docs MUST reference interaction IDs and data shapes defined here.
...
|---|---|
| **MCP-TOOL-SPEC (Doc 7)** | Interaction IDs, MCP tool names, MCP server assignments, input/output data shapes |
| **DESIGN-SPEC (Doc 8)** | Interaction IDs, Dashboard screen mappings, data shapes for rendering, journey wireframes |
| **DATA-MODEL (Doc 9)** | Data shape field definitions as source for DB schema |
| **API-CONTRACTS (Doc 10)** | REST endpoint names derived from interaction IDs, request/response shapes |
| **BACKLOG (Doc 11)** | Interaction IDs as acceptance-criteria anchors for each story |
| **TESTING (Doc 13)** | Interaction IDs as test-case identifiers for parity verification |
...
|------|-------------|-------------|----------|------------|-----------------|----------------|---------------|----------|
| I-001 | Trigger pipeline | Start a 14-doc generation run for a project | `trigger_pipeline` | agents-server | Pipeline Trigger Form (Pipeline Runs page) | `PipelineService.trigger()` | `PipelineRun` | F-001 |
...
interface PipelineRun {
  ...
  pipeline_name: string;         // e.g., "full-stack-first-14-doc"
  ...
  total_steps: number;           // total steps in pipeline (e.g., 14)
  current_step_name: string;     // e.g., "06-INTERACTION-MAP"
...
interface PipelineDocument {
  ...
  doc_number: number;            // 1-14 in the sequence
...
| 8 | Priya | MCP | Checks pipeline status, sees completion | I-002 | `PipelineRun` | Status = "completed", all 14 docs generated |
...
For MCP-TOOL-SPEC (Doc 7) convenience:
...
For DESIGN-SPEC (Doc 8) convenience:
...
Each interaction inherits NFR targets from QUALITY (Doc 4):
...
*End of INTERACTION-MAP. This document is the single source of truth for all interaction IDs, data shapes, and cross-interface contracts. MCP-TOOL-SPEC (Doc 7) and DESIGN-SPEC (Doc 8) MUST NOT define new interactions or shapes without updating this document first.*
