...
**Document:** 10 of 24 | Status: Draft
**Reads from:** INTERACTION-MAP (Doc 07) — all 22 data shapes drive table design. MCP-TOOL-SPEC (Doc 08) and DESIGN-SPEC (Doc 09) — query patterns drive index design.
...
All three consumers call the same shared service methods, which return the canonical data shapes defined in the INTERACTION-MAP (Doc 07). The database schema is designed so that every data shape can be materialized from table rows with minimal joins and no application-level post-processing.
...
CREATE TABLE pipeline_runs (
    ...
    total_steps     INTEGER NOT NULL DEFAULT 24 CHECK (total_steps > 0),
...
| 6 | MCP + Dashboard | `get_pipeline_documents` | I-006 | `SELECT * FROM pipeline_steps WHERE run_id = $1 AND status = 'completed' ORDER BY step_number` | idx_steps_run_id | 24 |
...
- **Size estimate:** ~500 KB per run (24 documents), ~2.5 GB/year at 10 runs/day.
