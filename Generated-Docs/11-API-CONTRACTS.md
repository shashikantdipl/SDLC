...
**Document:** 11 of 24 | Status: Draft
**Reads from:** INTERACTION-MAP (Doc 07), MCP-TOOL-SPEC (Doc 08), DESIGN-SPEC (Doc 09), ARCH (Doc 03)
...
1. **Wraps MCP tools** — Every one of the 35 MCP tools defined in MCP-TOOL-SPEC (Doc 08) has a REST equivalent. Non-AI consumers (CI/CD pipelines, scripts, third-party integrations) can perform every operation that an AI client performs via MCP, using standard HTTP. This satisfies quality requirement Q-049 (MCP/REST parity).

2. **Feeds the Dashboard** — The Streamlit Dashboard (DESIGN-SPEC, Doc 09) consumes REST endpoints for all five pages: Fleet Health, Cost Monitor, Pipeline Runs, Audit Log, and Approval Queue. Every widget and screen maps to one or more REST calls.
...
- Returns the same data shapes defined in INTERACTION-MAP (Doc 07)
...
Every MCP tool has a REST equivalent. This table maps all 35 MCP tools (from Doc 08) to REST endpoints (Q-049 compliance).
...
Every Dashboard screen (from DESIGN-SPEC, Doc 09) consumes one or more REST endpoints. This table guarantees all dashboard data needs are met.
...
All endpoints are prefixed with `/api/v1`. Request and response bodies use the data shapes defined in INTERACTION-MAP (Doc 07). Every endpoint references the interaction ID it implements and the shared service method it delegates to.
...
```json
{
  "data": {
    ...
    "project_id": "proj_abc123def456",
    "pipeline_name": "full-stack-first-24-doc",
    "status": "pending",
    "current_step": 0,
    "total_steps": 24,
    "current_step_name": "01-ROADMAP",
  ...
  "data": [
    {
      ...
      "current_step": 6,
      "total_steps": 24,
      "current_step_name": "07-INTERACTION-MAP",
      "started_at": "2026-03-24T14:30:00.000Z",
      "completed_at": null,
      "cost_usd": 8.42,
...
```json
{
  "data": {
    ...
    "approval_gates": ["07-INTERACTION-MAP", "11-API-CONTRACTS"]
  },
  "meta": {
...
```json
{
  ...
  "data": {
    ...
    "quality_score": 0.94,
    "next_step": 7,
    "next_step_name": "08-MCP-TOOL-SPEC",
