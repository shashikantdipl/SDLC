...
This document defines the `.claude/` directory structure, rules, and skills that **automatically enforce** the coding standards in CLAUDE.md (Doc 14) and the architectural constraints in ARCH.md (Doc 03). In the Full-Stack-First approach:
...
|----------|-------------------|
| CLAUDE.md (Doc 14) | Coding rules, forbidden patterns, repo structure, implementation patterns |
| ARCH.md (Doc 03) | 3 MCP servers, REST API (aiohttp), Dashboard (Streamlit), 8 shared services, PostgreSQL |
| INTERACTION-MAP (Doc 07) | Interaction IDs, data shape names, service method signatures |
| FEATURE-CATALOG (Doc 04) | Feature IDs referenced in rule justifications |

### Downstream Consumers

| Document | What It Reads From Here |
|----------|------------------------|
| TESTING (Doc 18) | Test scaffolding skills, coverage thresholds per layer |
| BACKLOG (Doc 13) | Enforcement setup stories, rule file deliverables |
...
TOOL_DEFINITION = {
    ...
    "description": "Start a 24-doc generation pipeline for a project",
    "input_schema": {
        "type": "object",
        "properties": {
            "project_id": {
...
- Names MUST match the INTERACTION-MAP (Doc 07) data shape definitions exactly.
...
## Parity Tests
- For every interaction in the INTERACTION-MAP (Doc 07), there MUST be a parity test.
