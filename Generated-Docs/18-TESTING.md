...
**Document:** 18 of 24 | Status: Draft
**Reads from:** ARCH (Doc 03), QUALITY (Doc 05), INTERACTION-MAP (Doc 07), MCP-TOOL-SPEC (Doc 08), DESIGN-SPEC (Doc 09), DATA-MODEL (Doc 10), API-CONTRACTS (Doc 11), BACKLOG (Doc 13)
...
| Service | `testcontainers-postgres` | 0.9.x | Real PostgreSQL for integration tests (no mocks for DB per CLAUDE Doc 14) |
...
class TestPipelineServiceIntegration:
    ...
    async def test_trigger_creates_pipeline_steps(self, service, clean_db):
        """trigger() should create 24 pipeline_steps rows for the run."""
        result = await service.trigger(
...
class TestPipelineRoutesUnit:
    ...
    async def test_post_pipelines_returns_201(self, client, mock_service):
        ...
        assert body["data"]["total_steps"] == 24
...
class TestJourney1PipelineApproval:
    ...
    async def test_full_pipeline_approval_journey(
        ...
        assert len(docs_result) == 24  # 24 documents
...
MCP_WRITE_TOOLS = [
    ("trigger_pipeline", {"project_id": "proj-perf", "pipeline_name": "full-stack-first-24-doc", "brief": "perf test"}),
...
class TestRestApiLatency:
    ...
    POST_ENDPOINTS = [
        ("/api/v1/pipelines", {"project_id": "proj-perf", "pipeline_name": "full-stack-first-24-doc", "brief": "perf"}),
...
Derived from QUALITY (Doc 05) NFRs Q-034 through Q-041.
...
def create_pipeline_run(
    ...
    pipeline_name: str = "full-stack-first-24-doc",
    status: str = "pending",
    current_step: int = 0,
    total_steps: int = 24,
    cost_usd: float = 0.0,
...
| `pipeline_steps` | 240 | 10 runs x 24 steps each |
