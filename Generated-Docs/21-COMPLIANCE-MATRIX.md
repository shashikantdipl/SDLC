...
**Document:** 21 of 24 | Status: Draft
**Reads from:** ARCH (Doc 03), QUALITY (Doc 05), DATA-MODEL (Doc 10), GUARDRAILS-SPEC (Doc 20), FAULT-TOLERANCE (Doc 19)
...
| SOC-PI-004 | PI1.4 | Output completeness | 24-doc pipeline produces all required documents; `pipeline_steps` table tracks each step completion | `pipeline_steps` WHERE `run_id = ?` AND `status = 'completed'` |
...
|-----------|----------------------|------------|------------------------|----------------|
| EUAI-001 | Art. 9 — Risk management system | Limited Risk (document generation tool) | NIST AI RMF implementation (Section 5); GUARDRAILS-SPEC (Doc 20) 4-layer guardrail system; quarterly risk reviews | Risk register + guardrail audit trail |
...
| EUAI-003 | Art. 11 — Technical documentation | Limited Risk | Full documentation set (24 docs); agent manifests with model versions; ARCH (Doc 03) system design | Generated-Docs/ directory; `agent_registry` table |
...
| EUAI-009 | Art. 69 — Codes of conduct | Limited Risk | ENFORCEMENT (Doc 15) defines agent behavior rules; agent rubrics (`rubric.yaml`) define scoring criteria | `agents/{id}/rubric.yaml` + ENFORCEMENT doc |
...
|-----------|-------------|-------------|------------------------|----------------|
| NIST-GOV-001 | GOVERN 1 | Policies and processes | ENFORCEMENT (Doc 15) rules; CLAUDE (Doc 14) coding standards; agent manifests define agent boundaries | Generated-Docs/15-ENFORCEMENT.md + manifests |
...
| NIST-GOV-003 | GOVERN 3 | Workforce diversity and expertise | 5 personas with distinct roles (Priya, Marcus, Jason, Anika, David, Fatima); cross-functional review in approval gates | PRD (Doc 02) persona definitions |
...
| Control ID | NIST AI RMF | Subcategory | Platform Implementation | Evidence Source |
|-----------|-------------|-------------|------------------------|----------------|
| NIST-MAP-001 | MAP 1 | Context and intended use | PRD (Doc 02) defines platform purpose; each agent manifest defines specific scope and constraints | Generated-Docs/02-PRD.md; agent manifests |
...
| NIST-MAP-004 | MAP 5 | Impact characterization | FAULT-TOLERANCE (Doc 19) defines impact per fault scenario; priority-based (P0/P1/P2) | Doc 19 fault definitions |

### 5.3 MEASURE Function

| Control ID | NIST AI RMF | Subcategory | Platform Implementation | Evidence Source |
|-----------|-------------|-------------|------------------------|----------------|
| NIST-MEA-001 | MEASURE 1 | Appropriate metrics identified | Quality dimensions: schema compliance, completeness, faithfulness, consistency (GR-OUT-005); cost per run; error rates | QUALITY (Doc 05) metrics |
...
| Control ID | NIST AI RMF | Subcategory | Platform Implementation | Evidence Source |
|-----------|-------------|-------------|------------------------|----------------|
| NIST-MAN-001 | MANAGE 1 | Risks prioritized and responded to | Fault tolerance 5-layer model (Doc 19); P0/P1/P2 prioritization; automated recovery for all P0 | FAULT-TOLERANCE doc |
| NIST-MAN-002 | MANAGE 2 | Risk mitigation planned and implemented | Guardrails (Doc 20) 4-layer mitigation; kill switches (GR-GOV-001); HITL gates (GR-GOV-002) | GUARDRAILS-SPEC doc |
...
|---|----------|----------|---------|-------|
| 1 | System architecture | `Generated-Docs/03-ARCH.md` | v1.0 | Jason |
| 2 | Data model | `Generated-Docs/10-DATA-MODEL.md` | v1.0 | Jason |
| 3 | API contracts | `Generated-Docs/11-API-CONTRACTS.md` | v1.0 | Jason |
| 4 | Fault tolerance | `Generated-Docs/19-FAULT-TOLERANCE.md` | v1.0 | David |
| 5 | Guardrails spec | `Generated-Docs/20-GUARDRAILS-SPEC.md` | v1.0 | Priya |
| 6 | This compliance matrix | `Generated-Docs/21-COMPLIANCE-MATRIX.md` | v1.0 | Fatima |
