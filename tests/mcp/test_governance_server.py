...
async def test_get_cost_report_calls_service(services):
    ...
    services["cost"].get_report.assert_awaited_once_with(scope="fleet", scope_id="", period_days=7)
...
async def test_check_budget_calls_service(services):
    ...
    services["cost"].check_budget.assert_awaited_once_with(scope="project", scope_id="proj_abc123def456")
    payload = json.loads(result[0].text)
...
async def test_get_cost_anomalies_calls_service(services):
    ...
    services["cost"].get_anomalies.assert_awaited_once_with(project_id="")
    payload = json.loads(result[0].text)
...
async def test_update_budget_threshold_calls_service(services):
    ...
    services["cost"].update_budget_threshold.assert_awaited_once_with(scope="project", scope_id="", new_budget=Decimal("0"))
    payload = json.loads(result[0].text)
...
async def test_query_audit_events_calls_service(services):
    ...
    services["audit"].query_events.assert_awaited_once_with(project_id="", severity=None, agent_id=None, limit=10, offset=0)
    payload = json.loads(result[0].text)
...
async def test_get_audit_summary_calls_service(services):
    ...
    services["audit"].get_summary.assert_awaited_once_with(project_id="", period_days=30)
    payload = json.loads(result[0].text)
...
async def test_export_audit_report_calls_service(services):
    ...
    services["audit"].export_report.assert_awaited_once_with(project_id="", period_days=30)
    payload = json.loads(result[0].text)
...
async def test_list_pending_approvals_calls_service(services):
    ...
    services["approval"].list_pending.assert_awaited_once_with(project_id=None)
    payload = json.loads(result[0].text)
    assert len(payload) == 1
...
async def test_approve_gate_calls_service(services):
    ...
    services["approval"].approve.return_value = {
        "approval_id": test_uuid,
        "status": "approved",
    }
    result = await handle_call_tool(
        "approve_gate",
        {"approval_id": test_uuid, "comment": "LGTM"},
    )
    services["approval"].approve.assert_awaited_once_with(
        approval_id=UUID(test_uuid), decision_by="mcp_user", comment="LGTM",
    )
    payload = json.loads(result[0].text)
...
async def test_reject_gate_calls_service(services):
    ...
    result = await handle_call_tool(
        "reject_gate",
        {"approval_id": test_uuid, "reason": "Missing security analysis for API endpoints"},
    )
    services["approval"].reject.assert_awaited_once_with(
        approval_id=UUID(test_uuid), decision_by="mcp_user", reason="Missing security analysis for API endpoints",
    )
    payload = json.loads(result[0].text)
...
async def test_approve_gate_not_found_returns_error(services):
    ...
    services["approval"].approve.side_effect = ValueError(f"Approval {test_uuid} not found")
    result = await handle_call_tool(
        "approve_gate",
        {"approval_id": test_uuid},
    )
